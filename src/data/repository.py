...
class Repository:
    ...
    async def get_all_speeches(self, date: datetime.date | None = None):
        ...
        async with self._factory() as session:
            ...
            speeches = result.all()
            self._update_speeches_slot_timezone(speeches)
            return list(map(self._mapper.map, speeches))
    ...
    def _update_speeches_slot_timezone(self, speeches: Iterable[Speech]):
        for speech in speeches:
            ...
            if slot.start_time.tzinfo is None:
                slot.start_time = slot.start_time.replace(tzinfo=self._timezone)
            if slot.end_time.tzinfo is None:
                slot.end_time = slot.end_time.replace(tzinfo=self._timezone)
